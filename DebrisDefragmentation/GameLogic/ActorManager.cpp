...
{
	///# 그냥 memset쓰던가 fill() 쓰면 되지.. 굳이 이렇게 루프 돌릴거까지야..
...
{
	// user id를 manager에 등록된 actor index와 같게 유지하면 객체들 인터랙션을 처리할 때 유리할 것 같은데 ///# 당연히 같게 유지하는 것이 좋지.
...
///# 아래 3 멤버 함수는 ISS의 기능을 호출하는걸 간접적으로 감싸 놓은 것에 불과... 그런 경우 네이밍을 확실하게.. TryIssOccupy... 
