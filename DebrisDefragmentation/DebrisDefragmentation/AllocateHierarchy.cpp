	...
	*ppNewMeshContainer = NULL;
	...
	if ( NumMaterials > 0 )
	{
		...
		for ( iMaterial = 0; iMaterial < NumMaterials; iMaterial++ )
		{
			...
			{
				std::string texturePath = ".\\Resources\\3DModel\\";
				texturePath.append( pMeshContainer->pMaterials[iMaterial].pTextureFilename );
				
				if ( FAILED( D3DXCreateTextureFromFileA( pd3dDevice, texturePath.c_str(),
...
HRESULT AllocateHierarchy::GenerateSkinnedMesh( IDirect3DDevice9* pd3dDevice, D3DXMESHCONTAINER_DERIVED* pMeshContainer )
{
	...
	if ( ( m_SkinningMethod == D3DINDEXEDHLSLVS ) )
	{
		// 본 개수를 가져온다-
		// Get palette size
		...
		/*
		// 여기서 할당할 필요가 없다. 실제로 쓰는 건 skinnedMesh
